...
pub fn ecdsa_verify_no_pubkey_check<'v, F: PrimeField, CF: PrimeField, SF: PrimeField, GA>(
    // Q: Why called GA? This is the subgroup of the elliptic curve? Is it "group affine?" But if that's the case, affine just referring to the space the coordinates are defined over?
    base_chip: &FpConfig<F, CF>,
    ctx: &mut Context<'v, F>,
    pubkey: &EcPoint<F, <FpConfig<F, CF> as FieldChip<F>>::FieldPoint<'v>>,
    r: &CRTInteger<'v, F>, // Q: shouldn't the CRTInteger r also be parameterised by the field it's in? This one, r, should be modulo n
    s: &CRTInteger<'v, F>,
    ...
    GA: CurveAffineExt<Base = CF, ScalarExt = SF>,
{
    ...
    let r_valid = scalar_chip.is_soft_nonzero(ctx, r); // Q: where do we assert that the basic CRT properties hold?
