use super::{CryptoRngCore, PlumeSignature, SecretKey, DST, ProjectivePoint, NonZeroScalar, PlumeSignatureV1Fields};
use k256::{elliptic_curve::{hash2curve::{GroupDigest, ExpandMsgXmd}, point::NonIdentity, sec1::ToEncodedPoint}, sha2::{Digest, Sha256}, Secp256k1,};
...
impl<'signing> PlumeSigner<'signing> {
    ...
    /// variant.
    pub fn new(secret_key: &SecretKey, v1: bool) -> PlumeSigner {PlumeSigner { secret_key, v1 }}
}
impl<'signing> RandomizedSigner<PlumeSignature> for PlumeSigner<'signing> {
    fn try_sign_with_rng(
        ...
        // Compute h = htc([m, pk])
        let hashed_to_curve = 
            NonIdentity::new(Secp256k1::hash_from_bytes::<ExpandMsgXmd<Sha256>>(
                &[msg, &pk_bytes], &[DST]
