...
/// @param {boolean} isV1 - is the flag to choose between V1 and V2 output.
...
pub fn sign(is_v1: bool, sk: &mut [u8], msg: &[u8]) -> Result<PlumeSignature, JsError> {
    ...
    sk.zeroize();
    let signer = plume_rustcrypto::randomizedsigner::PlumeSigner::new(&sk_z, is_v1);
    ...
    fn try_from(value: plume_rustcrypto::PlumeSignature) -> Result<Self, Self::Error> {
        Ok(PlumeSignature {
            instance: serde_wasm_bindgen::to_value(&PlumeSignaturePublic {
                ...
                s: plume_rustcrypto::SecretKey::from(value.s)
                    .to_sec1_der()
                    .expect(MSG_EXPECT)
                    .to_vec(),
